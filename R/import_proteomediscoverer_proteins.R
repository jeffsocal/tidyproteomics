#' A helper function for importing protein table data from ProteomeDiscoverer
#'
#' @param file_names a character vector of file paths
#'
#' @return a list data object
#'
import_pd_prots <- function(
    file_names = NULL
){
  ...
  cols_config <- path_to_package_data("proteomediscoverer_protein_headers") %>%
  ...
  for(i in 1:length(file_names) ){
    ...
    tryCatch({
      ...
      this_dat <- this_dat %>%
        ...
        tidyr::separate_rows(protein, num_peptides, num_unique_peptides, sep = "\\;\\s*") %>%
        dplyr::mutate(protein = sub("[a-z]{2,4}\\|", "", protein)) %>%
        dplyr::mutate(protein = sub("\\|[A-Z0-9]*", "", protein)) %>%
        dplyr::mutate(protein = sub("\\s+", " ", protein)) %>%
        tidyr::pivot_longer(dplyr::matches(cols_quantitative[[1]]),
                            names_to = 'sample', values_to = 'abundance_raw') %>%
        dplyr::mutate(sample_file = stringr::str_extract(sample, "F[0-9]*\\:\\s[0-9]{0,3}[CN]*") %>% tolower()) %>%
        dplyr::mutate(sample_file = sub("\\:", "", sample_file) %>% trimws()) %>%
        dplyr::mutate(sample_file = sub("\\s+", "_", sample_file)) %>%
      ...
      if(length(get_cols_impute) > 0){
        this_dat <- this_dat %>%
          ...
          dplyr::mutate(sample_file_mbr = sub("\\/", "_", sample_file_mbr)) %>%
  ...
  dl <- list(
    ...
    dat_out %>%
      codify(
        identifier = names(get_cols_identifiers),
        annotations = names(get_cols_annotations)
      ))
